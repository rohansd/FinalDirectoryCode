...
def test_object_sample():
    db_sampler = mmcv.ConfigDict({
        ...
        'info_path': './tests//data/cmpe249-fa22/kitti/kitti_dbinfos_train.pkl',
        'rate': 1.0,
        'prepare': {
    ...
    points = np.fromfile(
        './tests//data/cmpe249-fa22/kitti/training/velodyne_reduced/000000.bin',
        np.float32).reshape(-1, 4)
    annos = mmcv.load('./tests//data/cmpe249-fa22/kitti/kitti_infos_train.pkl')
    info = annos[0]
    ...
    expected_repr_str = 'ObjectSample sample_2d=False, ' \
                        'data_root=./tests//data/cmpe249-fa22/kitti/, ' \
                        'info_path=./tests//data/cmpe249-fa22/kitti/kitti' \
...
def test_affine_resize():
    ...
    results = dict()
    img = mmcv.imread('./tests//data/cmpe249-fa22/kitti/training/image_2/000000.png',
                      'color')
    results['img'] = img
